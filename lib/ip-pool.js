...
const createIpPool = async (ipAddresses, _iface, cfg) => {
	...
	const pool = new Pool({
		...
		createTimeoutMillis: addressAssignTimeout,
		destroyTimeoutMillis: addressRemoveTimeout,
		idleTimeoutMillis: addressMaxIdleTime,
		reapIntervalMillis: removeAddressesEvery,
	})
	...
	const onAbortSignal = () => {
		...
		debug('deleting all addresses in the pool', {
			numUsed: pool.numUsed(),
			numFree: pool.numFree(),
			numPendingCreates: pool.pendingCreates.length,
			numPendingValidations: pool.pendingValidations.length,
			numPendingAcquires: pool.pendingAcquires.length,
		})
		pool.destroy()
		.catch((err) => {
			debug('failed to destroy pool:', err)
		})

		// rtNetlinkSocket.socket.socket.close()
		// rtNetlinkSocket.socket.close()
	}
	process.on('SIGINT', onAbortSignal)
	process.on('SIGTERM', onAbortSignal)
	process.on('uncaughtExceptionMonitor', onAbortSignal)
