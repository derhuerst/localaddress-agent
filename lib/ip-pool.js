...
const parseAddress = (addr) => {
	...
	if (isIPv6(withoutMask)) {
		...
		address = parsed.addressMinusSuffix
		family = 4 // Node.js-style
		netlinkEncoded = {
			family: 2, // netlink-style
...
const deleteAddress = async (rtNetlink, encodedAddr, interfaceIdx) => {
	...
	// todo: wait until the address has been deleted
	// related: https://github.com/mildsunrise/node_netlink/issues/23
}
...
const createIpPool = async (ipAddresses, _iface, cfg) => {
	...
	const create = async () => {
		...
		const {address, family, netlinkEncoded} = parseAddress(_addr)
		try {
